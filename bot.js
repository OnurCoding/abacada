...
client.on("message", async msg => {
  ...
  let a = await db.fetch(`kufur_${msg.guild.id}`)
    if (a == 'acik') {
      const küfür = [
        "yarak","mk", "amk", "aq", "orospu", "oruspu", "oç", "sikerim", "yarrak", "piç", "amq", "sik", "amcık",  "sex", "seks", "amına", "orospu çocuğu", "sg", "siktir git","ananın amına yarak","ananı",
           "anneni","amcık","göt","porno","yarrağımın başı", "sikerim", "çük",  "Aq",  "ananı", "ananı sikeyim", "orospu evladı", "amına koyim",]
            if (küfür.some(word => msg.content.includes(word))) {
          try {
            if (!msg.member.hasPermission("MANAGE_GUILD")) {
                  msg.delete();
                          
                    return msg.reply(`🚫**Küfür Tespit Edildi**🚫`).then(msg => msg.delete(10000));
            }              
                } catch(err) {
                  console.log(err);
                }
...
client.on('message', message => {
  // Data
  let sistem = db.fetch(`pâyidarsaas_${message.guild.id}`)
...
.setTitle(`Aleyküm Selam Dostum, Hoşgeldin`) //Embed başlığını ayarlıyoruz.
.setDescription(`•Sunucumuza yeni katıldıysan +bilgi yazarak kanalları öğrenebilirsin.
•+yardım yazarak komutlar hakkında bilgi alabilirsin.
`) //Embed açıklamasını ayarlıyoruz.
.setColor(`RANDOM`) //Embed rengini ayarlıyoruz.

/* Sadece senin ihtiyacın olanları yazdım, tüm ayarları bu adresden görüntüleyebilirsin.
